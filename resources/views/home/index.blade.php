...
@section('content')
    <section class="rounded-2xl bg-gradient-to-r from-emerald-600 to-cyan-600 p-8 text-white">
        <h1 class="text-3xl font-bold">Marketplace Reservasi Tiket Wisata</h1>
        <p class="mt-2 max-w-2xl text-sm text-emerald-50">Temukan destinasi terbaik, pesan tiket online, dan scan QR saat tiba di lokasi.</p>
        <form action="{{ route('destinations.index') }}" class="mt-6">
    ...
    <section class="mt-10">
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-3">
            @forelse($recommendations as $destination)
                <a href="{{ route('destinations.show', $destination) }}" class="rounded-3xl bg-white p-6 shadow-sm hover:-translate-y-1 transition">
                    <div class="h-40 overflow-hidden rounded-3xl bg-slate-200">
                    ...
                    <h3 class="mt-4 text-lg font-semibold">{{ $destination->name }}</h3>
            ...
            @empty
                <div class="col-span-3 rounded-xl bg-white p-6 text-sm text-slate-500">Belum ada rekomendasi wisata.</div>
            @endforelse
        </div>
    </section>
    ...
    <section class="mt-10 rounded-2xl bg-white p-8 shadow-sm">
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-3">
            <div class="rounded-3xl border border-slate-200 p-5">
                ...
                <p class="mt-2 text-sm text-slate-500">Cari dan pilih wisata yang ingin dikunjungi di halaman Wisata.</p>
            </div>
            <div class="rounded-3xl border border-slate-200 p-5">
