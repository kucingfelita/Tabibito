...
@foreach($destinations as $destination)
    <div class="group relative bg-white rounded-3xl border border-slate-100 p-4 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
        <div class="flex flex-col md:flex-row gap-6">
            <!-- Image Section -->
            <div class="relative shrink-0 w-full md:w-64 h-48 md:h-40">
                <a href="{{ route('destinations.show', $destination) }}" class="block h-full w-full overflow-hidden rounded-2xl">
                    @if($destination->images->first()?->image_path)
                        <img src="{{ asset('storage/' . $destination->images->first()->image_path) }}" alt="Destinasi Wisata {{ $destination->name }} di {{ $destination->city }} - Tabibito Jateng" class="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500">
                    @else
                ...
                <!-- Wishlist Button (Inside relative image wrapper) -->
                @auth
                    ...
                    <button onclick="toggleWishlist(event, {{ $destination->id }})" 
                            ...
                            class="absolute -top-2 -right-2 w-10 h-10 rounded-xl flex items-center justify-center transition-all shadow-lg z-10 
                            {{ $isWishlisted ? 'bg-primary-600 text-white shadow-primary-200' : 'bg-white text-slate-400 hover:text-primary-600 shadow-slate-200' }}">
                        <svg class="w-5 h-5 {{ $isWishlisted ? 'fill-current' : 'fill-none' }}" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                    </button>
                @endauth
            </div>
            ...
            <div class="flex-1 flex flex-col justify-between min-w-0 py-1">
                <div>
                    ...
                    <a href="{{ route('destinations.show', $destination) }}" class="block">
                        <h2 class="text-xl font-bold text-slate-900 group-hover:text-primary-600 transition-colors truncate mb-2">{{ $destination->name }}</h2>
                    </a>
