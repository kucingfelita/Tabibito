                    ...
                    @if(is_null($trx->rating))
                        <div class="mt-4 pt-4 border-t border-slate-100">
                            <form action="{{ route('history.rating', $trx) }}" method="POST" enctype="multipart/form-data" class="space-y-4" x-data="{ rating: 0, hoverRating: 0, imagePreview: null }">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-4">
                                        <label class="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 hover:border-primary-500 rounded-2xl p-4 cursor-pointer transition-colors bg-slate-50 hover:bg-slate-100/50 w-32 h-32 shrink-0">
                                            ...
                                            <span class="text-[10px] font-bold text-slate-400 mt-2">Pilih Foto</span>
                                            <input type="file" name="review_image" class="hidden" accept="image/*" 
                                                   @change="const file = $event.target.files[0]; if (file) { const reader = new FileReader(); reader.onload = (e) => { imagePreview = e.target.result; }; reader.readAsDataURL(file); }">
                                        </label>
                                        
                                        <template x-if="imagePreview">
                                            <div class="relative w-32 h-32 rounded-2xl overflow-hidden border border-slate-100 shadow-sm bg-slate-50">
                                                <img :src="imagePreview" class="w-full h-full object-cover">
                                                <button type="button" @click="imagePreview = null; $event.target.closest('.flex').querySelector('input[type=file]').value = ''" 
                                                        class="absolute top-1.5 right-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-full p-1 shadow-md transition-colors">
                                ...
                                <div>
                                    <button type="submit" class="rounded-lg bg-amber-500 hover:bg-amber-600 px-6 py-2.5 text-sm font-bold text-white transition-colors shadow-lg shadow-amber-500/30">Kirim Penilaian & Ulasan</button>
                                </div>
                            </form>
                        </div>
                    @else
                        <div class="mt-4 pt-4 border-t border-slate-100 flex flex-col gap-2">
                            ...
                            @if($trx->review_comment)
                                <p class="text-sm text-slate-600 bg-slate-50 rounded-2xl p-4 border border-slate-100/50 mt-1 italic">
                                    "{{ $trx->review_comment }}"
                                </p>
                            @endif
                            
                            @if($trx->review_image)
                                <div class="mt-2 w-36 h-36 rounded-2xl overflow-hidden border border-slate-100 shadow-sm bg-slate-50">
