...
@section('content')
    ...
    <form method="POST" action="{{ route('owner.tickets.store') }}" class="mt-4 grid gap-3 rounded-xl bg-white p-4 shadow-sm md:grid-cols-2">
        ...
        <input name="name" placeholder="Nama paket tiket" class="rounded-lg border px-3 py-2">
        <input type="number" name="price" placeholder="Harga" class="rounded-lg border px-3 py-2">
        <input type="number" name="daily_quota" placeholder="Kuota harian" class="rounded-lg border px-3 py-2">
        ...
        @foreach($tickets as $ticket)
            <div class="rounded-xl bg-white p-4 shadow-sm">
                <form method="POST" action="{{ route('owner.tickets.update', $ticket) }}" class="grid gap-3 md:grid-cols-2">
                    ...
                    <input name="name" value="{{ $ticket->name }}" class="rounded-lg border px-3 py-2">
                    <input type="number" name="price" value="{{ $ticket->price }}" class="rounded-lg border px-3 py-2">
                    <input type="number" name="daily_quota" value="{{ $ticket->daily_quota }}" class="rounded-lg border px-3 py-2">
